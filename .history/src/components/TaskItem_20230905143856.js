    ...
    return (
        ...
        <div className='task-container'>
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <div className='status'  style={{backgroundColor: task.isComplete ? '#ff9b9b' : 'green' }}>
                    <input type='checkbox' onChange={(e) => onStatusChange(task.id, e.target.checked)} />
                    <label style={{ fontSize: 'small' , paddingRight: '10px'}}>{task.isComplete ? 'Completed' : 'Not Completed'}</label>
                </div>
