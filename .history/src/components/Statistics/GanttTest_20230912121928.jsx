...
const GanttTest = () => {
    ...
    const getProgressPercent = (start_date, due_date, completion_date) => {
        if (
            moment(completion_date).locale("en-GB") >= moment(start_date).locale("en-GB") &&
            moment(completion_date).locale("en-GB") < moment(due_date).locale("en-GB")
        ) {
            const totalNumberOfDays = moment(due_date).locale("en-GB").diff(
                moment(start_date).locale("en-GB"),
                "days"
            );

            let taskCompletedInDays = moment(completion_date).locale("en-GB").diff(
                moment(start_date).locale("en-GB"),
                "days"
            );
